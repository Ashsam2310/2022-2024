...
gpg = gnupg.GPG()
# generate key
input_data = gpg.gen_key_input(name_email='test@email.com',passphrase='passphrase',)
...
with open('test123.txt', 'rb') as f:
    status = gpg.encrypt_file(fileobj_or_path=f,recipients=['test@email.com'],output='encrypted.txt.gpg',)
print(status.ok)
print(status.status)
print(status.stderr)
print('~'*50)
...
with open('encrypted.txt.gpg', 'rb') as f:
    status = gpg.decrypt_file(fileobj_or_path=f,passphrase='passphrase',output='decrypted.txt',)
print(status.ok)
print(status.status)
print(status.stderr)